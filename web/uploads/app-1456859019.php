...
class adid_adv_entity_details extends Baseobject 
{
...
foreach($data as $prop=>$value) {
    ...
    }    private function setIsAnaMembeer($is_ana_membeer) {
        $this->is_ana_membeer = $is_ana_membeer;
    ...
    }    private function setSourc($sourc) {
        $this->sourc = $sourc;
    }
...
class adid_adv_entity_detailsTable extends Baseobject{
    public static function get($id, $data=null) {
		return new adid_adv_entity_details($id, $data);
    }
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare("SELECT * FROM adid_adv_entity_details  LIMIT $offset,$limit;");
	...
	//$stmt->bindValue(':is_ana_membeer', 'is_ana_membeer', PDO::PARAM_STR);
	...
	//$stmt->bindValue(':sourc', 'sourc', PDO::PARAM_STR);
		$stmt->execute();
	$objs = array();
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('INSERT INTO adid_adv_entity_details ( nielsen_advertiser_code,nielsen_advertiser_entity,sum_of_total_spend,spend_category,primary_pcc,uses_adid,start_year,advertiser_loyalty,monthly_loyalty,is_ana_membeer,is_ana_board_member,in_nielsen,sourc) VALUES(:nielsen_advertiser_code,:nielsen_advertiser_entity,:sum_of_total_spend,:spend_category,:primary_pcc,:uses_adid,:start_year,:advertiser_loyalty,:monthly_loyalty,:is_ana_membeer,:is_ana_board_member,:in_nielsen,:sourc');
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('UPDATE adid_adv_entity_details SET nielsen_advertiser_code=:nielsen_advertiser_code,nielsen_advertiser_entity=:nielsen_advertiser_entity,sum_of_total_spend=:sum_of_total_spend,spend_category=:spend_category,primary_pcc=:primary_pcc,uses_adid=:uses_adid,start_year=:start_year,advertiser_loyalty=:advertiser_loyalty,monthly_loyalty=:monthly_loyalty,is_ana_membeer=:is_ana_membeer,is_ana_board_member=:is_ana_board_member,in_nielsen=:in_nielsen,sourc=:sourc');
	...
	$stmt->bindValue(':is_ana_membeer', $changes['is_ana_membeer'], PDO::PARAM_STR);
	...
	$stmt->bindValue(':sourc', $changes['sourc'], PDO::PARAM_STR);
		$stmt->execute();
...
$offset=$offset*$limit;
$objs = adid_adv_entity_detailsTable::getAll($offset,$limit);
...
$ct=adid_adv_entity_detailsTable::getCount();
$mod = $ct%$limit;
