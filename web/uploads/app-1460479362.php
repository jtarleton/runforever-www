...
class Baseobject extends Database {
	public function __construct() {
		$dsn = sprintf('mysql://%s:%s@%s/%s','adiddw2', 'adiddw2', 'localhost', 'adid_dw_test');
...
class CodesCreatedReport extends Baseobject 
{
public $cid;
	public $pid;
	public $bid;
...
class CodesCreatedReportTable extends Baseobject{
    public static function get($id, $data=null) {
		return new CodesCreatedReport($id, $data);
    }
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare("SELECT * FROM CodesCreatedReport  LIMIT $offset,$limit;");
...
$offset=$offset*$limit;
$objs = CodesCreatedReportTable::getAll($offset,$limit);
...
$ct=CodesCreatedReportTable::getCount();
$mod = $ct%$limit;
