...
class churn_account_status extends Baseobject 
{
public $bid;
	public $name;
...
class churn_account_statusTable extends Baseobject{
    public static function get($id, $data=null) {
		return new churn_account_status($id, $data);
    }
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare("SELECT * FROM churn_account_status  LIMIT $offset,$limit;");
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('INSERT INTO churn_account_status ( bid,name,locked_unlocked,parent,status,balance,balance_status) VALUES(:bid,:name,:locked_unlocked,:parent,:status,:balance,:balance_status');
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('UPDATE churn_account_status SET bid=:bid,name=:name,locked_unlocked=:locked_unlocked,parent=:parent,status=:status,balance=:balance,balance_status=:balance_status');
...
$offset=$offset*$limit;
$objs = churn_account_statusTable::getAll($offset,$limit);
...
$ct=churn_account_statusTable::getCount();
$mod = $ct%$limit;
