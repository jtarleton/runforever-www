...
class nielsen_spend extends Baseobject 
{
public $nsid;
	public $advertiser;
	public $pcc_ext;
...
foreach($data as $prop=>$value) {
    ...
    }    public function getPccExt() {
        return $this->pcc_ext;
    }    private function setNsId($nsid) {
        $this->nsid = $nsid;
    }    private function setAdvertiser($advertiser) {
        $this->advertiser = $advertiser;
    }    private function setPccExt($pcc_ext) {
        $this->pcc_ext = $pcc_ext;
    }
...
class nielsen_spendTable extends Baseobject{
    public static function get($id, $data=null) {
		return new nielsen_spend($id, $data);
    }
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare("SELECT * FROM nielsen_spend  LIMIT $offset,$limit;");
	...
	//$stmt->bindValue(':pcc_ext', 'pcc_ext', PDO::PARAM_STR);
		$stmt->execute();
	$objs = array();
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('INSERT INTO nielsen_spend ( nsid,advertiser,pcc_ext) VALUES(:nsid,:advertiser,:pcc_ext');
...
$pdo = parent::Connect();
	...
	$stmt=$pdo->prepare('UPDATE nielsen_spend SET nsid=:nsid,advertiser=:advertiser,pcc_ext=:pcc_ext');
	...
	$stmt->bindValue(':pcc_ext', $changes['pcc_ext'], PDO::PARAM_STR);
		$stmt->execute();
...
$offset=$offset*$limit;
$objs = nielsen_spendTable::getAll($offset,$limit);
...
$ct=nielsen_spendTable::getCount();
$mod = $ct%$limit;
